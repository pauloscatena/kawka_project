...
- **Premissa de ACL do achado #8 parece incorreta** (QA Task 10): o achado original da Buttercup (#8) dizia que o recreate "descarta as ACLs" e o tópico volta world-writable. A semântica documentada do Kafka é o oposto: ACLs são recursos independentes do ciclo de vida do tópico (deletar o tópico não deleta a ACL), então ACLs literais no mesmo nome **persistem** no tópico recriado. O aviso de ACL foi **removido** do formulário em vez de afirmar o oposto sem prova. Antes de adicionar qualquer aviso sobre ACL, verificar contra um cluster com `authorizer.class.name` configurado — e, se confirmado, o gotcha correto a comunicar é que permissões antigas persistem, não que somem.
